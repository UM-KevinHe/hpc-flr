...
inline double logdet_info(const arma::vec& info_gamma,             // m-vector (diag D)
                   const arma::mat& schur)
{
  ...
  if (!arma::chol(R, S)) {
    ...
    if (!arma::chol(R, S + ridge * arma::eye(S.n_rows, S.n_cols))) {
      throw std::runtime_error("Cholesky failed: Schur complement not (numerically) PD.");
    }
  }
...
inline arma::vec rep(arma::vec &x, arma::vec &each) {
...
// Scalar overload: repeat the value `x`, `each` times.
inline arma::vec rep(double x, double each) {
  return x * arma::ones<arma::vec>(static_cast<arma::uword>(each));
}

inline double Loglkd(const arma::vec &Y, const arma::vec &Z_beta, const arma::vec &gamma_obs) {
...
inline double Loglkd_firth(arma::vec &Y, arma::mat &Z, arma::vec &n_prov, 
...
inline FirthFit logis_firth_prov(arma::vec &Y, arma::mat &Z, arma::vec &n_prov, arma::vec gamma, arma::vec beta,
                      int n_obs, int m, const int &threads, double tol = 1e-5, int max_iter = 100,
                      double bound = 10.0, bool message = false, const std::string &stop = "beta",
    ...
    while (iter < max_iter && crit > tol) {
        ...
        #pragma omp single
        {
            ...
            if (stop == "beta"){
                ...
                if (message == true) {
                    std::cout << "Iter " << iter << ": Inf norm of running diff in est reg parm is " << setprecision(3) << scientific << crit << ";" << endl;
                }
            } else {
                ...
                if (stop == "relch") {
                    ...
                    if (message == true) {
                        std::cout << "Iter " << iter << ": Relative change in est log likelihood is " << setprecision(3) << scientific << crit << ";" << endl;
                    }
                } else if (stop == "ratch") {
                    crit = abs(d_loglkd/(loglik_pen-pen_init));
                    if (message == true) {
                        std::cout << "Iter " << iter << ": Adjusted relative change in est log likelihood is " << setprecision(3) << scientific << crit << ";" << endl;
                    }
                } else if (stop == "all") {
                    arma::vec crits(3);
                    ...
                    crit = crits.max();
                    if (message == true) {
                        std::cout << "Iter " << iter << ": Maximum criterion across all checks is " << setprecision(3) << scientific << crit << ";" << endl;
                    }
                } else if (stop == "or") {
                    arma::vec crits(3);
                    crits(0) = norm(d_beta, "inf");
                    ...
                    crit = crits.min();
                    if (message == true) {
                        std::cout << "Iter " << iter << ": Minimum criterion across all checks is " << setprecision(3) << scientific << crit << ";" << endl;
                    }
                } else {
                    throw std::runtime_error("Argument 'stop' NOT as required!");
                }
            }
        }
        #pragma omp barrier
  ...
  return FirthFit{ gamma, beta, loglik_pen, loglik_trace, iter, crit };
}
...
// ---------------------------------------------------------------------------
inline arma::vec wald_gamma(arma::mat &Z, arma::vec &p, arma::vec &n_prov, arma::vec &parm_indices) {
