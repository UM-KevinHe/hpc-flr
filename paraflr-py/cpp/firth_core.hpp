// firth_core.hpp — pure C++ core for parallel Firth logistic regression.
//
// This is the SAME math as paraflr/src/firth.cpp, with the R-specific layer
// removed so it can be bound from Python (pybind11) exactly as it is bound
// from R (Rcpp). Only three things differ from firth.cpp:
//   * Rcpp::List return  -> the FirthFit struct below
//   * Rcpp::Rcout logging -> an optional std::function logger
//   * Rcpp::stop(...)     -> throw std::runtime_error(...)
// The numerical code is identical, so an R fit and a Python fit agree to
// numerical tolerance.  Header-only: each binding just #includes it.
#pragma once
...
// Optional logger. R binding routes this to Rcpp::Rcout; Python can pass a
// print callback or leave it empty (the library never prints on its own).
...
inline arma::vec rep(const arma::vec &x, const arma::vec &each) {
...
inline double loglkd(const arma::vec &Y, const arma::vec &Z_beta,
...
inline void ind2uppsub(unsigned int index, unsigned int dim,
...
inline arma::mat info_beta_omp(const arma::mat &Z, const arma::vec &pq,
                               const int &threads) {
...
inline FirthFit logis_firth_prov(const arma::vec &Y, arma::mat Z,
        const arma::vec &n_prov, arma::vec gamma, arma::vec beta,
        int n_obs, int m, const int &threads, double tol = 1e-5,
        int max_iter = 10000, double bound = 10.0, bool backtrack = false,
        const Logger &log = nullptr) {
  int iter = 0, n = n_obs, ind = 0;
  double v, crit = 100.0;
  double s = 0.01, t = 0.6, lambda = 0.0, d_loglkd = 0.0, loglkd_v = 0.0;
  ...
  while (iter < max_iter) {
    ...
    if (log) log("Iter " + std::to_string(iter) + ": crit = " + std::to_string(crit));
  }
  if (log) log("Firth converged after " + std::to_string(iter) + " iterations.");
  return FirthFit{ gamma, beta, iter };
}

inline double logdet_info(const arma::vec &info_gamma, const arma::mat &schur) {
  ...
  if (!arma::chol(R, S)) {
    ...
    if (!arma::chol(R, S + ridge * arma::eye(S.n_rows, S.n_cols)))
      throw std::runtime_error("Cholesky failed: Schur complement not PD.");
  }
  return logdetA + 2.0 * arma::sum(arma::log(R.diag()));
}

inline double loglkd_firth(const arma::vec &Y, arma::mat Z,
        const arma::vec &n_prov, const arma::vec &gamma, const arma::vec &beta) {
...
inline arma::vec wald_gamma(arma::mat Z, const arma::vec &p,
        const arma::vec &n_prov, const arma::vec &parm_indices) {
  ...
  for (int i = 0; i < m; i++) {
    double sm = sum(pq(arma::span(ind, ind + n_prov(i) - 1)));
    if (sm < eps) return na_vec;
    info_gamma_inv(i) = 1.0 / sm;
